/* ------------------------------------------------------------------
 L’Oréal Product-Aware Routine Builder — Client Script (GitHub Pages ready)
 ...
 - Description modal, search, RTL toggle
 - Generate Routine with local fallback (no backend required)
------------------------------------------------------------------- */
...
function mapCat(c){
  ...
  if (['cleanser','toner','serum','moisturizer','sunscreen','makeup','haircare','fragrance'].includes(x)) return x;
  if (x === 'skincare') return 'serum';
  if (x === 'suncare') return 'sunscreen';
  if (x === 'hair styling' || x === 'hair color' || x === "men's grooming") return 'haircare';
  return x || 'serum';
}
...
function productCard(p){
  ...
  const img = p.image ? `<img alt="" src="${escapeHtml(p.image)}" style="width:100%;height:140px;object-fit:contain;background:#0a0d12;border-bottom:1px solid var(--ring)">` : '';
  return `
    <article class="product ${isSelected ? 'selected':''}" data-id="${p.id}">
      ...
      <footer>
        <button class="select-btn" data-action="toggle">${isSelected?'Unselect':'Select'}</button>
...
function renderSelected(){
    ...
    ? items.map(p => `
        <li class="selected-item" data-id="${p.id}">
          <div>
            ...
            <small>${escapeHtml(p.brand || 'L’Oréal')} • ${escapeHtml(mapCat(p.category))}</small>
          </div>
  ...
  localStorage.setItem('selectedIds', JSON.stringify(Array.from(STATE.selectedIds)));
}
...
// ---------- Chat ----------
function greet(){
  addMsg('assistant', 'Hi! Pick a few products, then hit “Generate Routine.” Ask follow-ups about order, when to use, and frequency.');
}
...
function onTopic(text){
  ...
  return /(skin|face|acne|spf|sunscreen|cleanser|toner|serum|moisturizer|retinol|niacinamide|vitamin c|makeup|hair|shampoo|conditioner|fragrance|perfume|loreal|garnier|lancome|routine|oily|dry|combination|sensitive|pores|hyperpigmentation|texture|mask|exfoliat)/.test(s);
}
...
function stripProduct(p){
  return {
    id:String(p.id), name:p.name, brand:p.brand, category:mapCat(p.category),
...
function buildLocalRoutine(selected){
  ...
  const extras = selected.filter(p=>!['cleanser','toner','serum','moisturizer','sunscreen'].includes(mapCat(p.category)));

  const stepsAM = [];
  const stepsPM = [];

  if(cleanser) { stepsAM.push(step(cleanser)); stepsPM.push(step(cleanser)); }
  if(toner) { stepsAM.push(step(toner)); stepsPM.push(step(toner)); }
  if(serum) { stepsAM.push(step(serum)); stepsPM.push(step(serum)); }
  if(moisturizer) { stepsAM.push(step(moisturizer)); stepsPM.push(step(moisturizer)); }
  if(sunscreen) { stepsAM.push(step(sunscreen, 'last, 2-finger rule (SPF 30+)')); }

  const makeup = extras.filter(p=>mapCat(p.category)==='makeup').map(step);
  const haircare = extras.filter(p=>mapCat(p.category)==='haircare').map(step);
  const fragrance = extras.filter(p=>mapCat(p.category)==='fragrance').map(step);

  const am = bullets([
    ...stepsAM,
    ...makeup.length ? ['(Optional) Makeup: then apply selected makeup.'] : []
  ]);
  const pm = bullets(stepsPM);

  const reminders = [
    !sunscreen ? 'No sunscreen selected — consider adding SPF 30+ for AM.' : null,
...
async function handleChat(e){
  ...
  if(!onTopic(q)){
    addMsg('assistant', 'Let’s keep it beauty-focused (skincare, haircare, makeup, fragrance).');
    els.chatInput.value = '';
    return;
  }
  ...
  if(workerUrl && /^https?:\/\//.test(workerUrl)){
    try{
      const res = await fetch(workerUrl, {
        ...
        body: JSON.stringify({
          messages: STATE.chat,
          products: STATE.products.filter(p=>STATE.selectedIds.has(String(p.id))).map(stripProduct),
          web: !!els.liveWebToggle?.checked
        })
      });
      if(res.ok){
...
// ---------- Utils ----------
function badge(text){ return `<span class="tag">${escapeHtml(text || '')}</span>`; }
function escapeHtml(s){ return String(s).replace(/[&<>"']/g, m => ({'&':'&amp;','<':'&lt;','>':'&gt','"':'&quot;',"'":'&#39;'}[m])); }
